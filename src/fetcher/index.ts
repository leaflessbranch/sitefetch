...
import {
  TransformerFactory,
  serializePages as transformSerializePages,
} from "../transforms"
import { ProgressTracker, ProgressEvent, type ProgressStats } from "../progress"
...
export class Fetcher {
  ...
  #setupProgressEvents(): void {
    ...
    if (typeof this.options.progress?.onProgress === "function") {
      this.#progressTracker.on(
        ...
        this.options.progress.onProgress
      )
    }

    if (typeof this.options.progress?.onPageFetched === "function") {
      this.#progressTracker.on(
        ProgressEvent.PAGE_FETCHED,
        this.options.progress.onPageFetched
      )
    }

    if (typeof this.options.progress?.onPageFailed === "function") {
      this.#progressTracker.on(
        ProgressEvent.PAGE_FAILED,
        this.options.progress.onPageFailed
      )
    }
    ...
    if (typeof this.options.progress?.onComplete === "function") {
      this.#progressTracker.on(
        ...
        this.options.progress.onComplete
      )
    }
  }
  ...
  async fetchSite(
    ...
    if (checkpointId && this.options.resume?.enabled) {
      try {
        ...
        const checkpoint = await this.#resumeHandler.loadCheckpoint(
          checkpointId
        )
        ...
        this.#fetched = new Set(
          checkpoint.processedUrls.concat(
        ...
        for (const pendingUrl of checkpoint.pendingUrls) {
          this.#queue.add(() =>
            this.#fetchPage(pendingUrl, { skipMatch: false })
          )
        }
        ...
        for (const failedUrl of checkpoint.failedUrls) {
          this.#progressTracker.pageFailed(
            failedUrl,
            new Error("Failed in previous run")
          )
        }

        // Report skipped pages to progress tracker
        for (const skippedUrl of checkpoint.skippedUrls) {
          this.#progressTracker.pageSkipped(
            skippedUrl,
            "Skipped in previous run"
          )
        }

        logger.info(
          `Resumed from checkpoint with ${this.#pages.size} pages and ${
            checkpoint.pendingUrls.length
          } pending URLs`
        )
      } catch (error) {
  ...
  async #fetchPage(
    ...
    try {
          ...
          if (cachedPage) {
            ...
            if (options.skipMatch) {
              try {
                ...
                logger.warn(
                  `Error processing links from cached page: ${error.message}`
                )
              }
            }
      ...
      if (resUrl.host !== host) {
        ...
        this.#progressTracker.pageSkipped(
          url,
          `Redirected to different host: ${resUrl.host}`
        )
      ...
      if (this.options.pagination?.enabled) {
        const paginationUrls = await this.#paginationHandler.detectPagination(
          $,
          url,
          htmlContent
        )
        if (paginationUrls.length > 0) {
          logger.info(
            `Found ${paginationUrls.length} pagination URLs for ${url}`
          )
      ...
      if (!html) {
        ...
        this.#progressTracker.pageFailed(
          url,
          new Error("No readable content found")
        )
      ...
      if (!article) {
        this.#progressTracker.pageFailed(
          url,
          new Error("Failed to parse content with Readability")
        )
      ...
      if (this.options.transform?.format) {
        try {
          ...
          if (format.toLowerCase() === "html") {
            ...
            const transformer = TransformerFactory.getTransformer(
              format.toLowerCase()
            )
            if (transformer) {
      ...
      if (this.options.pagination?.enabled) {
        paginationMetadata = this.#paginationHandler.extractPaginationMetadata(
          $,
          url
        )
      }
      ...
      if (this.options.metadata) {
        try {
          ...
          if (page.metadata?.dates?.published) {
            logger.info(
              `Found publication date: ${page.metadata.dates.published}`
            )
          }
        } catch (error) {
          logger.warn(
            `Failed to extract metadata from ${url}: ${error.message}`
          )
        }
      }
      ...
      if (this.options.filter) {
        ...
        if (!includePage) {
          ...
          this.#progressTracker.pageSkipped(
            url,
            "Filtered out by content filters"
          )
  ...
  updateRateLimitOptions(
    options: Partial<import("../types").RateLimitOptions>
  ): void {
   ...
   */
  updateFilterOptions(
    options: Partial<import("../types").FilterOptions>
  ): void {
  ...
  updateMetadataOptions(
    options: Partial<import("../types").MetadataOptions>
  ): void {
  ...
  updateRequestOptions(
    options: Partial<import("../types").RequestOptions>
  ): void {
   ...
   */
  updateResumeOptions(
    options: Partial<import("../types").ResumeOptions>
  ): void {
